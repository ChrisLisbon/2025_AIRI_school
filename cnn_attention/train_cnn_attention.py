...
general_folder = f'{sea_name}_cnn_att_2020({batch})'
weights_folder = f'{sea_name}_cnn_att_2020({batch})/weights'
images_folder = f'{sea_name}_cnn_att_2020({batch})/images'
...
model = CNNAttention(channels_in=pre_history_size, out_channels=forecast_size)
print(model)
model.to(device)
...
model_name = f'{weights_folder}/cnn_att_{sea_name}_{start_date}-{end_date}.pt'
...
for epoch in range(max_epochs):
    ...
    if epoch % 10 == 0:
        ...
        plt.ylabel(f'L1Loss')
        plt.legend()
        plt.title('Convergence CNN-attention')
