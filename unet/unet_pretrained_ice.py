...
general_folder = f'{sea_name}_unet_raw_2020({batch})'
weights_folder = f'{sea_name}_unet_raw_2020({batch})/weights'
images_folder = f'{sea_name}_unet_raw_2020({batch})/images'
...
model = smp.Unet('resnet34', in_channels=pre_history_size)
...
model_name = f'{weights_folder}/unet_{sea_name}_{start_date}-{end_date}.pt'
...
for epoch in range(max_epochs):
    ...
    if epoch % 10 == 0:
        ...
        plt.ylabel(f'L1Loss')
        plt.legend()
        plt.title('Convergence U-Net (raw)')
